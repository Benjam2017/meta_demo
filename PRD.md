...
- **冷启动营销推送能力**（用户从未联系过、商家主动推送欢迎消息+选项）——详见 9.1，目前完全未实现，且需要先补一个持久化层才能做。

### 9.1 需求草案：冷启动营销推送（Proactive Marketing Push）

> 背景：用户提出"想主动给从未联系过的用户推送欢迎消息+选项按钮"。这与当前 demo 已实现的"客服窗口内被动回复"是两套不同的机制（前者需要审核模板，后者不需要），现状代码完全不支持前者，记录如下，暂不实施。

**为什么不能直接复用现有的 `sendTryOutDemoMessage`**：`GraphApi.messageWithInteractiveReply`（`services/graph-api.js:50-73`）发的是自由格式交互消息，只能在用户最近 24 小时内主动联系过你的"客服窗口"内发送。对从未联系过的用户，Meta 会直接拒绝这类调用——必须改用预先审核通过的**模板消息**，且按钮要用模板的 `QUICK_REPLY` 按钮类型，而不是 `interactive.action.buttons`。

**完整管理闭环包含 6 个必须项：**

1. **Opt-in 合规**：发送名单必须是用户在某个渠道（官网表单、线下登记等）明确同意接收 WhatsApp 营销消息的号码，不能凭空拉名单群发——这是 Meta 政策强制要求，违反会触发限速/封号。
2. **模板生命周期管理**：起草 → 提交审核（WhatsApp Manager 网页或 `POST /{WABA_ID}/message_templates`）→ `PENDING`/`APPROVED`/`REJECTED` 状态跟踪 → 上线后持续监控模板质量评分（对应 webhook 字段 `message_template_quality_update`，见附录 A.4.4）。
3. **Messaging Tier 限速**：账号有"24 小时内可联系的不同用户数"上限（新账号通常从 250 起步），随发送质量逐步升级，批量推送必须按 tier 限速排队，不能一次性全量发送。
4. **批量发送的持久化与重试**：需要存储 opt-in 名单、每次发送的 `wamid`、发送状态，失败要重试或从名单剔除——**当前仓库没有任何数据库**，`services/redis.js` 只是 15 秒 TTL 的临时标记，不能复用为名单存储。
5. **送达跟踪扩展**：复用 `Conversation.handleStatus` 的"按 `wamid` 匹配状态"模式（`services/conversation.js:133-151`），但要从"仅触发一次性跟进消息"扩展为"持久更新这条推送记录的送达/已读/失败状态"。
6. **退订处理**：用户回复 "STOP" 等退订关键词时，需要在 `Conversation.handleMessage` 里识别并把该号码从名单中移除——平台强制要求，不处理视为违规。

**结论**：这是一个独立于当前 demo 范围的新功能，最低限度需要先引入一个持久化层（哪怕是 SQLite/本地 JSON 起步）才能开始实现，不是在现有 `services/` 代码上加一个函数就能完成的。
